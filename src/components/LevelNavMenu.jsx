...
export const LevelNavMenu = ({levels, current_level_index, highest_unlocked_level, show_level_nav, page}) => {
  ...
  let buttons = levels.map((level, index) => {
    ...
    return <LevelNavButton display_in_nav_menu={index >= lower_index && index < upper_index} show_level_nav={true} key={`${level.id}-NavButton`} level={level} index={index} current_level_index={current_level_index} locked_class={locked_class} current_level_class={current_level_class}/>;
  })

  return(
    <div className='LevelNavMenu' style={{ display: show_level_nav ? null : 'none', margin:0 }}>
      <div className='row'>
